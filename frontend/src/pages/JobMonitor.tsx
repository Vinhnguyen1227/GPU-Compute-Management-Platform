...
import { ArrowLeft, Terminal, Cpu, Flame, HardDrive, Square, Play, RefreshCw, CheckCircle, AlertOctagon } from 'lucide-react';
...
export const JobMonitor: React.FC<JobMonitorProps> = ({ job, onBack, onCancelJob }) => {
  ...
  useEffect(() => {
    // Initial static logs
    const initLogs = [
      `[INFO] [2026-08-10 04:30:01] Job ${job.id} initialized by Java Job Scheduler.`,
      `[INFO] [2026-08-10 04:30:02] Matching requirements: ${job.gpuType} x${job.gpuCount}`,
      `[INFO] [2026-08-10 04:30:05] Assigned to worker node: ${job.assignedNodeId || 'node-h100-01'}.`,
      `[INFO] [2026-08-10 04:30:10] Loading framework container: ${job.framework}`,
      `[INFO] [2026-08-10 04:30:15] Executing command: $ ${job.command}`,
      `[CUDA] Initializing NCCL distributed backend across ${job.gpuCount} devices...`,
      `[INFO] Epoch 1/3 - Batch 100/1000 - Loss: 2.8421 - VRAM: 54.2 GB / 80 GB`,
      `[INFO] Epoch 1/3 - Batch 500/1000 - Loss: 1.9420 - VRAM: 58.1 GB / 80 GB`,
      `[INFO] Epoch 2/3 - Batch 200/1000 - Loss: 1.2104 - VRAM: 61.4 GB / 80 GB`,
    ];
    setLogs(initLogs);
    ...
    // Simulate streaming logs every 3 seconds
      ...
      <div>
        ...
        <div className="glass-panel p-6 rounded-2xl border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          {job.status === 'RUNNING' && (
            ...
            >
              <Square className="w-3.5 h-3.5 fill-current" /> Terminate Job Execution
            </button>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-panel p-5 rounded-2xl border border-slate-800">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="text-base font-bold text-white">Training Loss & Telemetry Curve</h2>
              <p className="text-xs text-slate-400 font-mono">Real-time convergence telemetry</p>
            </div>
            <div className="flex items-center gap-4 text-xs font-mono">
              <span className="text-[#76B900]">● Training Loss</span>
              <span className="text-cyan-400">● GPU Util %</span>
            </div>
          </div>
          <div className="h-64">
        ...
        <div className="glass-panel p-5 rounded-2xl border border-slate-800 flex flex-col justify-between">
          <h2 className="text-base font-bold text-white mb-4">Node Telemetry Gauge</h2>
          <div className="space-y-4 font-mono">
            <div className="p-3 bg-slate-900/60 rounded-xl border border-slate-800 flex items-center justify-between">
              <span className="text-xs text-slate-400 flex items-center gap-2">
                <Cpu className="w-4 h-4 text-cyan-400" /> GPU Compute Load
              </span>
            ...
            <div className="p-3 bg-slate-900/60 rounded-xl border border-slate-800 flex items-center justify-between">
              <span className="text-xs text-slate-400 flex items-center gap-2">
                <HardDrive className="w-4 h-4 text-purple-400" /> VRAM Memory Used
              </span>
          ...
          <div className="mt-4 pt-3 border-t border-slate-800 text-[11px] font-mono text-slate-500 flex items-center justify-between">
            ...
            <span className="text-[#76B900]">Worker Service Active</span>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel rounded-2xl border border-slate-800 overflow-hidden">
        <div className="px-5 py-3 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center gap-2 text-xs font-mono text-slate-300">
            ...
            <span>Worker Process Terminal Output (stdout/stderr)</span>
          </div>
